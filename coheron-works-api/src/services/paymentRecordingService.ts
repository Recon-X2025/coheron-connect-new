...
export class PaymentRecordingService {
  async recordPayment(tenantId: string, invoiceId: string, amount: number, paymentMethod: string, reference?: string, date?: Date): Promise<any> {
    return withTransaction(async (session) => {
      ...
      await Invoice.updateOne({ _id: invoiceId }, { $set: { payments, amount_paid: totalPaid, payment_status: newStatus, status: totalPaid >= invoice.amount_total ? 'paid' : invoice.status } }, { session });
