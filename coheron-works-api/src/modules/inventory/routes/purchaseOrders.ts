...
router.post('/', authenticate, validate({ body: createPurchaseOrderSchema }), asyncHandler(async (req: any, res: any) => {
...
router.put('/:id', authenticate, validate({ params: objectIdParam, body: updatePurchaseOrderSchema }), asyncHandler(async (req: any, res: any) => {
...
router.post('/:id/confirm', authenticate, validate({ params: objectIdParam }), asyncHandler(async (req: any, res: any) => {
  if (!req.user?.tenant_id) {
    return res.status(400).json({ error: 'Tenant ID is required' });
  }
...
router.post('/:id/receive', authenticate, validate({ params: objectIdParam }), asyncHandler(async (req: any, res: any) => {
  if (!req.user?.tenant_id) {
    return res.status(400).json({ error: 'Tenant ID is required' });
  }
...
router.post('/:id/create-bill', authenticate, validate({ params: objectIdParam }), asyncHandler(async (req: any, res: any) => {
  if (!req.user?.tenant_id) {
    return res.status(400).json({ error: 'Tenant ID is required' });
  }
